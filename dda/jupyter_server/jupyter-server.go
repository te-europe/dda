...
func DeployJupyterServer(c *cli.Context) error {
	...
	if response == "y" {
		...
		containerResourceId := dr.Properties.OutputResources[0].Id
		// Print the output

		connectionString, err := az.GetJupyterServerAddress(containerResourceId)
		if err != nil {
			log.Fatal(err)
		}
		...
		t := table.NewWriter()
		t.SetOutputMirror(os.Stdout)
		t.AppendHeader(table.Row{"Resource", "Connection String"})
		t.AppendRow(table.Row{"Jupyter Server", connectionString})
